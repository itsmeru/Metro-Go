...
class MetroSystem:
    def __init__(self, lines: List[Tuple[str, List[str]]], station_names: Dict[str, str], transfer_stations: Dict[str, List[str]], branch_points: Dict[str, List[str]], transfer_times: Dict[str, int]):
        self.lines = lines
        ...
        for station, codes in self.transfer_stations.items():
            for i in range(len(codes)):
                for j in range(i+1, len(codes)):
                    graph[codes[i]].append((codes[j], "轉乘", 2))
                    graph[codes[j]].append((codes[i], "轉乘", 2))

        for station, branches in self.branch_points.items():
            for branch in branches:
                graph[station].append((branch, "分叉", 0))
                graph[branch].append((station, "分叉", 0))
    ...
    def get_previous_station(self, station: str, line: str, direction_code: str):
        ...
        if station_index == line_stations[0] or station_index == line_stations[-1]:
    ...
    async def get_next_trains(self, station: str, line: str, direction: str, direction_code: str, current_time: datetime, redis):
        ...
        if not next_trains:
            ...
            next_trains.append({
                ...
                'station_id': station,
                'waiting_time': 420,  # 7分鐘默認等待時間
                'travel_time': 0,
                'total_time': 420,
                'arrival_time': current_time + timedelta(seconds=420)
            })
    ...
    async def process_paths_with_waiting_times(self, path: List[List[str]], redis, start_time: datetime):
            ...
            while i < len(path)-1:
                ...
                if i == 0 or line in ["轉乘", "分叉"]:
                    if line in ["轉乘", "分叉"]:
                        transfer_time = self.transfer_times.get(self.station_names[next_station], 5)*60 

                        # 設置轉乘時間，如果沒有特定時間則默認為5分鐘
                        transfer_time = transfer_time or 300  
            ...
            if station not in visited:
                ...
                for next_station, line, edge_cost in self.graph[station]:
                    if next_station not in new_path:
                        ...
                        total_cost = cost + edge_cost + travel_time
...
# 分叉點數據
branch_points = {
    "O12": ["O13", "O50"], # 假設 O14 站有一個分支到 O50
    "R22": ["R23","R22A"],# 假設 R22 站有一個分支到 R22A
    "G03": ["G03A","G04"] 
}
...
async def get_travel_plan(start,end):
    metro = MetroSystem(lines, station_names, transfer_stations, branch_points, transfer_times)
